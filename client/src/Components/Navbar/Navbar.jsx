...
const Navbar = () => {
    ...
    // Handle keyboard navigation and selection with looping behavior
    const handleKeyDown = (e) => {
        if (documents.length === 0) return;

        if (e.key === 'ArrowDown') {
            // Move down the list, loop back to the top if at the end
            setHighlightedIndex((prevIndex) => (prevIndex + 1) % documents.length);
        } else if (e.key === 'ArrowUp') {
            // Move up the list, loop to the bottom if at the top
            setHighlightedIndex((prevIndex) => (prevIndex - 1 + documents.length) % documents.length);
        } else if (e.key === 'Enter') {
