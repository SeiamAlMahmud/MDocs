    ...
    return (<>
        <div className='navbar flex  items-center px-3 sm:px-10 md:px h-20 justify-between bg-pink-400 border-b-8 border-sky-500 backdrop-blur-sm'>
            <div className='flex gap-3 items-center justify-start'>
                ...
                <h2 className='hidden sm:block text-3xl'>MDocs</h2>
                </div>
            <div className="right flex justify-end items-center gap-4">
                <div className="search_icon w-[40vw] md:w-[35vw] relative">
                    <i onClick={() => setToggle(prev => !prev)} className={`absolute cursor-pointer top-2 transition-transform duration-300 ease-in-out text-3xl ${toggle ? "left-8 transform -translate-x-[90%]" : "right-1 translate-x-0"}`}>{toggle ? <RxCrossCircled className='text-white bg-black rounded-full hover:rotate-180 transition-all duration-300' /> : <RiSearchLine />}</i>
