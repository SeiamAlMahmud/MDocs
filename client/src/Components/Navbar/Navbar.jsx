...
const Navbar = () => {
    ...
    const debouncedUpdate = useCallback(
        debounce((search) => {
            if (search.trim() === "") {
                setDocuments([]);  // Clear documents when search is blank
            } else {
                fetchSearch(search)
            }
        }, 500),
        ...
        <div className={`navbar z-[100] flex  items-center px-3 sm:px-10 md:px h-14 py-8  md:h-20 justify-between ${location.pathname == "/" ? "bg-[#02124f]" : "bg-[#fcc419]"}  border-b-8  : "text-black"} border-black backdrop-blur-sm z-50 `}>
            ...
            <div className="right flex justify-end items-center gap-4 ">
                <div className="search_icon w-[50vw] md:w-[35vw] relative">
                    <i onClick={() => {
                        setToggle(prev => !prev)
                    }} className={`absolute text-white  cursor-pointer top-2 transition-transform duration-300 ease-in-out text-3xl ${toggle ? "left-8 transform -translate-x-[90%]" : "right-1 translate-x-0"}`}>{toggle ? <RxCrossCircled className='text-white bg-black rounded-full hover:rotate-180 transition-all duration-300' /> : <RiSearchLine />}</i>
                    <input
                        type="text"
                        value={searchDocs}
                        onChange={(e) => {
